...
def get_boltz_command(
    ...
    if settings.get("use_msa_server"):
        ...
        if (
            settings.get("msa_server_url")
            and settings["msa_server_url"] != DEFAULTS["msa_server_url"]
        ):
    ...
    if settings.get("sampling_steps_affinity"):
        cmd.extend(
            ["--sampling_steps_affinity", str(settings["sampling_steps_affinity"])]
        )
    ...
    if settings.get("diffusion_samples_affinity"):
        cmd.extend(
            [
                ...
                str(settings["diffusion_samples_affinity"]),
            ]
        )
...
def create_input_builder_tab():
        ...
        * **🎯 Affinity Prediction**: Predict binding affinity (pIC50) between a Binder and Target.
        ...
        with gr.Accordion("📂 Batch Converter (Multi-FASTA)", open=False):
            ...
            **Convert a Multi-FASTA file into individual YAML files.**
            ...
            with gr.Row():
                batch_fasta = gr.File(
                    label="Upload Multi-FASTA", file_types=[".fasta", ".fa"]
                )
        ...
        with gr.Accordion("🧬 Combinatorial Peptide-Receptor Docking", open=False):
            gr.Markdown("""
            **Generate all combinations of peptide-receptor pairs.**
            Upload two FASTA files and get N×M YAML files for bulk docking.
            
            - Chain A = Receptor
            - Chain B = Peptide
            """)
            with gr.Row():
                peptide_fasta = gr.File(
                    label="Upload Peptide FASTA", file_types=[".fasta", ".fa"]
                )
                receptor_fasta = gr.File(
                    label="Upload Receptor FASTA", file_types=[".fasta", ".fa"]
                )

            enable_affinity_checkbox = gr.Checkbox(
                label="🎯 Enable Affinity Prediction",
                value=False,
                info="Add affinity properties to all YAMLs (Peptide as binder)",
            )

            combinatorial_btn = gr.Button(
                "🔄 Generate Combinatorial YAMLs", variant="secondary"
            )
            combinatorial_output = gr.File(label="Download ZIP", visible=False)
    ...
    ).then(fn=lambda: gr.update(visible=True), outputs=[batch_output])

    def convert_combinatorial(peptide_file, receptor_file, enable_affinity):
        """Generate combinatorial YAML files from two FASTA files."""
        if not peptide_file or not receptor_file:
            return None

        from gui.yaml_builder import convert_combinatorial_fasta

        try:
            zip_path = convert_combinatorial_fasta(
                peptide_file, receptor_file, enable_affinity=enable_affinity
            )
            return zip_path
        except Exception as e:
            print(f"Error in combinatorial conversion: {e}")
            return None

    combinatorial_btn.click(
        fn=convert_combinatorial,
        inputs=[peptide_fasta, receptor_fasta, enable_affinity_checkbox],
        outputs=[combinatorial_output],
    ).then(fn=lambda: gr.update(visible=True), outputs=[combinatorial_output])
    ...
    def add_sequence(
        ...
        try:
            if entity_type == "protein":
                ...
                if smiles and ccd:
                    return (
                        ...
                        properties,
                        gr.update(),
                        "❌ Provide only SMILES or CCD, not both",
                    )
                entry = create_ligand_entry(
                    chain_id,
                    smiles=smiles if smiles else None,
                    ccd=ccd if ccd else None,
                )
            ...
            yaml_str = build_yaml(
                sequences, properties=properties if properties else None
            )
            ...
            return (
                ...
                properties,
                yaml_str,
                f"✅ Added {entity_type} chain {chain_id}",
            )
...
def create_advanced_settings_tab():
    ...
    all_settings = [
        ...
        adv_num_subsample,
        adv_mw_correction,
        adv_affinity_sampling,
        adv_affinity_samples,
        adv_format,
        adv_write_pae,
        adv_write_pde,
        adv_write_embed,
    ...
    def run_advanced(input_file, output_dir, *settings):
        ...
        if not input_file:
            return (
                "**Status:** ❌ Please upload an input file",
                "",
...
def create_results_tab():
    ...
    with gr.Column():
        ...
        pred_dropdown.change(
            fn=on_dropdown_select, inputs=[pred_dropdown], outputs=[results_dir]
        )

        def refresh_list():
            return gr.update(choices=list_predictions())

        refresh_btn.click(fn=refresh_list, inputs=[], outputs=[pred_dropdown])
    ...
    def analyze_results(folder_path):
        ...
        for pred_name, pred_path in dirs_to_check:
            ...
            structure_files = glob.glob(
                os.path.join(pred_path, "*_model_*.cif")
            ) + glob.glob(os.path.join(pred_path, "*_model_*.pdb"))
            ...
            if structure_files:
                results_md += (
                    f"**📁 Structure files:** {len(structure_files)} model(s)\n\n"
                )
...
"""
            ...
            for aff_file in aff_files:
                try:
                    ...
                    binding = (
                        "✅ Likely binder" if prob > 0.5 else "❌ Likely non-binder"
                    )
